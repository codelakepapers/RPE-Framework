...
def construct_model(cfg: Schema, list_create_mlp, rgnn=True, **kwargs): # a list of mlp creators
    ...
    if cfg.pe_method == "sign_inv":
        ...
        if not rgnn:
            ...
            return Model_PEARL(
                cfg.n_node_types, cfg.node_emb_dims,
                positional_encoding=PEARL_PE1(phi=gin, rho=rho),
                base_model=base_model,
                pe_aggregate=cfg.pe_aggregate,
                feature_type=kwargs.get("feature_type")
            )
        ...
        if not rgnn:
            ...
            return Model_PEARL(
                cfg.n_node_types, cfg.node_emb_dims,
                positional_encoding=PEARL_PE2(phi=gin, rho=rho),
                base_model=base_model,
                pe_aggregate=cfg.pe_aggregate,
                feature_type=kwargs.get("feature_type")
            )
        ...
        if cfg.pe_method == 'pearl':
            pe_model = K_PEARL_PE(Phi, Psi_list, cfg.BASIS, k=cfg.RAND_k, mlp_nlayers=cfg.RAND_mlp_nlayers, mlp_hid=cfg.RAND_mlp_hid, spe_act=cfg.RAND_act, mlp_out=cfg.RAND_mlp_out)
...
'''class GINBaseModel(nn.Module):
    gin: GIN

    def __init__(
        self, n_layers: int, in_dims: int, hidden_dims: int, create_mlp: Callable[[int, int], MLP],
            residual: bool = False, bn: bool = False, pooling: str = "mean",
            target_dim: int = 1) -> None:
        super().__init__()
        self.gin = GIN(n_layers, in_dims, hidden_dims, hidden_dims, create_mlp, residual=residual,
                         bn=bn)
        self.mlp = create_mlp(hidden_dims, target_dim)
        self.pooling = global_mean_pool if pooling == 'mean' else global_add_pool
    ...
    def forward(
        ...
        :return: Predicted regression values. [B]
        """
        X_n = self.gin(X_n, edge_index, PE)   # [N_sum, hidden_dim]
        # X_n = global_mean_pool(X_n, batch) # [B, hidden_dim]
        X_n = self.pooling(X_n, batch) # [B, hidden_dim]
        Y_pred = self.mlp(X_n)         # [B, 1]
        return Y_pred.squeeze(dim=1) '''
